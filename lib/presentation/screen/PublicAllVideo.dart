...
class _LayoutState extends State<PublicAllVideo>
  ...
  Widget getBody(widgetContext) {
    var size = MediaQuery.of(widgetContext).size;
              ...
              children: [
                for (int i = 0; i < widget.p!.data!.length; i++)
                  VideoPlayerItem(
                    ...
                    likes:
                        '${int.parse(widget.p!.data![i].likes_count!) + LikesCount}',
                    user_like: widget.p!.data![i].user_like,
                    comments:
                        '${widget.p!.data![i].comments!.length + CommentCount}',
                    repprt: 'ابلاغ',
                    albumImg: '',
                    ...
                    addrepprt: () {
                      ...
                      Navigator.of(widgetContext).push(MaterialPageRoute(
                          builder: (context) => repprt_screen(
                                title: 'ابلاغ عن محتوي',
                              )));
                    },
                    addFollow: () {
                      addFollow(i);
  ...
  void addFollow(int i) {
    BlocProvider.of<NadekCubit>(context)
        .AddFollow(token: token!, uid: widget.p!.data![i].owner!.id!.toInt());
  }
  ...
  void addLike(int itemIndex) {
    ...
    BlocProvider.of<NadekCubit>(context).AddLike(
        token: token!, map: {'video_id': widget.p!.data![itemIndex].id});
  }
                    ...
                    Expanded(
                      ...
                      child: Loadeing
                          ...
                          : ListView.builder(
                              ...
                              itemCount:
                                  commentsModel?.data?.comments?.length ?? 0,
                              itemBuilder: (context, index) {
                                return ListTile(
                                  title: Text(
                                    commentsModel?.data?.comments?[index].user
                                            ?.name ??
                                        'non user',
                                    style: const TextStyle(color: Colors.white),
                                  ),
                                  subtitle: Text(
                                      commentsModel?.data?.comments?[index]
                                              .comment ??
                                          'non comment',
                                      style:
                                  ...
                                  leading: CircleAvatar(
                                    ...
                                    backgroundImage: NetworkImage(
                                        '${commentsModel?.data?.comments?[index].user?.photo}'),
                                  ),
                                );
                              },
                            ),
      ...
      child: SingleChildScrollView(
        ...
        children: <Widget>[
          getProfile(profileImg,
              function: addFollow, profileClick: profileClick),
          getIconsLike(
              icon: Icon(
                Icons.favorite,
          ...
          getIcons(
            ...
            () {
              Share.share(
                  'لينك الفيديو \n $video \n لمشاهدة المزيد يمكنك تحميل تطبيق ناديك \n ${string_app.nadekplayStore}');
            },
          ),
