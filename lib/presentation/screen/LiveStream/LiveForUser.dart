...
class _LiveForUserState extends State<LiveForUser> with WidgetsBindingObserver{
  ...
  void addUserToLive(){
    _firestore.collection('UserToLive').doc(textUuid).collection(textUuid)
        .add({
      'addUser' : false,
      'msgAdd'  :false,
      'total'   :0,
  ...
  void addLive(){
    _firestore.collection('StreamLive').doc("Live").collection("ChannelsId").add({
      'channels' : textUuid,
      'title' : name,
      'imageUrl' :photo,
      'time' : FieldValue.serverTimestamp(),
  ...
  void addViews(){
    _firestore.collection('ViewInLive').doc(textUuid).collection(textUuid!)
        .add({
      'view' : 0,
      'time' : FieldValue.serverTimestamp(),
    });
  ...
  void deleteUidForLive(){
    _firestore.collection('StreamLive').doc("Uid").collection(textUuid).get().then((value) {
      value.docs.forEach((element) {
        FirebaseFirestore.instance.collection('StreamLive').doc("Uid").collection(textUuid).doc(element.id).delete();
  ...
  void Delete()async{
    var c=  FirebaseFirestore.instance
        .collection('StreamLive').doc("Live").collection("ChannelsId");
    var shot =c.get().then((value){
      value.docs.forEach((element) {
        print(element.data()["channels"] + '=='+textUuid );
        if(element.data()["channels"] == textUuid){
        FirebaseFirestore.instance.collection('StreamLive').doc("Live").collection("ChannelsId").doc(element.id).delete();
         deleteViews();
         deleteUserToLive();
         deleteUidForLive();
        }
  ...
  void deleteViews()async{
    var c=  FirebaseFirestore.instance
        .collection('ViewInLive').doc(textUuid).collection(textUuid!);
    var shot =c.get().then((value){
      value.docs.forEach((element) {
        FirebaseFirestore.instance.collection('ViewInLive').doc(textUuid).collection(textUuid!).doc(element.id).delete();
  ...
  void deleteUserToLive()async{
    var c=  FirebaseFirestore.instance
        .collection('UserToLive').doc(textUuid).collection(textUuid);
    var shot =c.get().then((value){
      value.docs.forEach((element) {
        FirebaseFirestore.instance.collection('UserToLive').doc(textUuid).collection(textUuid!).doc(element.id).delete();
  ...
  void declinedToLive(){
    var c=  FirebaseFirestore.instance.collection('UserToLive').doc(textUuid).collection(textUuid);
    var shot =c.get().then((value){
      value.docs.forEach((element) {
        FirebaseFirestore.instance.collection('UserToLive').doc(textUuid).collection(textUuid).doc(element.id).update({
          'addUser' : false,
          'msgAdd'  :false,
          'total'   :0,
          'name': 'non',
    ...
    return Scaffold(
      key: _scaffoldKey,
        ...
        body: StreamBuilder<QuerySnapshot>(
            stream: _firestore.collection('UserToLive').doc(textUuid).collection(textUuid).snapshots(),
                ...
                child: Column(
                  children: [
                    Container(
                        ...
                        child:Stack(
                          children: [
                            snapshot.data!.docs[0].get("addUser")?  Row(
                              children: [
                                Expanded(
                                  child: _remoteVideo(),
                                ),
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          IconButton(
                              ...
                              icon: Icon(
                                enableLocalVideo?Icons.videocam:Icons.videocam_off,
                                size: 35,
                                color: Colors.white,
                              )
                          ),
  ...
  void showInSnackBar(String title) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ...
      ScaffoldMessenger.of(_scaffoldKey.currentContext!).showSnackBar(s).closed.then((value){
       // declinedToLive();
      });
  ...
  Widget BOx(){
    return  StreamBuilder<QuerySnapshot>(
      stream:_firestore.collection('ViewInLive').doc(textUuid).collection(textUuid!).snapshots() ,
  ...
  Widget StraemComments(){
    return Container(
      ...
      child: StreamBuilder<QuerySnapshot>(
        stream: _firestore.collection('StreamLive').doc(textUuid).collection("LiveComments").orderBy('time').snapshots(),
        builder: (context, snapshot) {
          ...
          return messagesWidgets.isEmpty? Center(
            child: Text(
              'هنا تظهر التعليقات',
