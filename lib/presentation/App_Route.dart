  ...
  Route? create_app_route(RouteSettings settings) {
    switch (settings.name) {
      ...
      case '/ProfileOfUser':
        ...
        return MaterialPageRoute(
            builder: (_) => ProfileOfUser(myProfile: false, user_id: data[0]));
      case '/FollowMe':
        return MaterialPageRoute(builder: (_) => const FollowMe());
      case '/report':
        return MaterialPageRoute(
            builder: (_) => repprt_screen(title: 'ابلاغ عن محتوي'));
      case '/virtualtournmentsList':
        return MaterialPageRoute(builder: (_) => VirtualTournmentsList());
      case '/livePage':
        var data = settings.arguments as List;
        return MaterialPageRoute(
          builder: (_) => LivePage(data[0], data[1], data[2]),
        );
    }
    return null;
  }
